...
import { ApiClient, type RemoteArticle, type RemoteBook } from "../api";
...
function bookNote(book: RemoteBook, linkStyle: "markdown" | "wikilink", ext: string): string {
  const lines: string[] = [
    "---",
    "ps-readonly: true",
    "---",
    ...
    `# ${book.title}`,
    "",
    "> Read-only book index generated by ps-sync. Edit the chapter files, not this list.",
    "",
  ];
  let currentPart: string | null = null;
  for (const ch of book.chapters) {
    if (ch.partTitle && ch.partTitle !== currentPart) {
      currentPart = ch.partTitle;
      lines.push(`## ${currentPart}`, "");
    }
    // Book notes live at <pub>/books/, chapters at <pub>/articles/.
    const link =
      linkStyle === "wikilink"
        ? `[[${ch.articleSlug}|${ch.title}]]`
        : `[${ch.title}](../articles/${ch.articleSlug}.${ext})`;
    lines.push(`${ch.position + 1}. ${link}`);
  }
  lines.push("");
  return lines.join("\n");
}
  ...
  for (const pub of publishers) {
    ...
    if (!values["no-books"]) {
      ...
      for (const b of books) {
        ...
        const abs = join(root, path);
        mkdirSync(dirname(abs), { recursive: true });
        writeFileSync(abs, bookNote(book, config.links, ext));
      }
